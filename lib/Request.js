...
var debug = require('debug')('res:request');
...
Request.prototype.fetch = function (poll) {
  ...
  if (this._poll) {
    // if in poll mode, call '_reset' after fetch
    // in order to reset the poll timer
    done = function __done () {
  ...
  if (this.validate()) {
    ...
    return Request.go(this.options, done);
  }
...
Request.prototype.poll = function () {
  this.fetch(true);
};
